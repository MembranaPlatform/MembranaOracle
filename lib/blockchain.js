...
module.exports = class {
  ...
  preprocContractDetails({contractDetails}){
    return {
      currentState: parseInt(contractDetails.currentState),
      start: parseInt(contractDetails.start),
      deadline: parseInt(contractDetails.deadline),
      maxLoss: parseInt(contractDetails.maxLoss),
      startBalance: parseInt(contractDetails.startBalance),
      targetBalance: parseInt(contractDetails.targetBalance),
      amount: contractDetails.amount,
      currency: parseInt(contractDetails.currency),
      ...
      offerId: contractDetails.offerId,
      dealId: parseInt(contractDetails.dealId),
    };
  }
  async eventProcessing({lastBlock}){
    try {
      ...
      for (let i of dealsEvents) {
        ...
        let contractDetails = await this.dealsContract.methods.deals(dealId).call();
        contractDetails.offerId = offerId;
        contractDetails.dealId = dealId;
        contractDetails = this.preprocContractDetails({contractDetails});
        ...
        if (state === 0){
          ...
          await this.newDealCallback(contractDetails);
        }
      }
